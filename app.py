...
def extract_live_events_from_atp_page(url, page_label):
    ...
    month_blocks = re.split(r"(January|February|March|April|May|June|July|August|September|October|November|December),?\s+2026", text)
    ...
    for part in month_blocks:
        ...
        for match in re.finditer(r"([A-Z][A-Za-z\-\.\' ]+),\s*([A-Z][A-Za-z\-\.\' ]+)\s+(\d{1,2})-(\d{1,2})\s+" + current_month[:3] + r",\s*2026\s+([^\.]{3,90})", part):
            ...
            try:
                ...
                start = date(2026, month_num, day1)
            except Exception:
                continue
            surface = "Hard"
            label = page_label
...
def fetch_live_itf_calendar(limit=40):
    ...
    pattern = re.compile(r"([A-Z][A-Za-z\-\.\' ]+),\s*([A-Z][A-Za-z\-\.\' ]+)\s*\|\s*(\d{1,2})\s*-\s*(\d{1,2})\s*([A-Za-z]{3})\s*2026")
...
# =========================
    ...
    review_sport = st.selectbox(
        ...
        ["Tennis", "Running", "Gym / strength", "Baseball", "Rowing", "Weightlifting", "Water Polo", "Soccer", "Boxing"]
    )
...
# =========================
    ...
    if counselling_mode == "Tennis":
        ...
        st.write("Helping a tennis player choose the most logical tournament for the next 7 days, using official ATP, Challenger, and ITF sources when available.")
        ...
        if st.button("Generate tennis tournament advice", use_container_width=True):
            ...
            st.caption(f"Window analyzed: {format_date(start_window)} to {format_date(end_window)}")
